...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
	static const uint8_t ucIPAddress[ 4 ] = { 192, 168, 0, 80 };
	static const uint8_t ucNetMask[ 4 ] = { 255, 255, 255, 0 };
	static const uint8_t ucGatewayAddress[ 4 ] = { 192, 168, 0, 1 };
	static const uint8_t ucDNSServerAddress[ 4 ] = { 8, 8, 4, 4 };

	static uint8_t ucMACAddress [ 6 ] = {0x00, 0x15, 0x60, 0xC0, 0xC4, 0x26};
  /* USER CODE END 1 */
...
BaseType_t xApplicationGetRandomNumber( uint32_t * pulNumber ){
	/* #TODO# add an actual random number generator that returns a 32 bit number*/
...
/*#TODO# Create an actual function that generates random TCP initial sequence numbers*/
extern uint32_t ulApplicationGetNextSequenceNumber( uint32_t ulSourceAddress,uint16_t usSourcePort, uint32_t ulDestinationAddress,uint16_t usDestinationPort )
{
     ...
     uint32_t ulNumber;
     return xApplicationGetRandomNumber(&(ulNumber));
}
